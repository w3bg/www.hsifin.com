...
// YOU WILL NEED TO CHANGE OUT THE TWO COL_ID_#'S BELOW IF YOU HAVE DIFFERENT MATRIX IDS THAN THE ONES LISTED
	
//	Major Modifications and customization by Tori Sheppeard
	
//	Certain script functions utilized from :
	...
	$.ajax({
		...
		success: function(xml) {
			...
			$(xml).find('element').each(function(){
				...
				order[x] = $(this).find('row_order').text();
				//CHANGE OUT THE COL ID BELOW FOR THE ONE THAT MATCHES YOUR PARTICULAR MATRIX FIELD ID IN THE DB FOR THE TEXT STRING
				var textStr = $(this).find('col_id_39').text();
				var splitTxt = textStr.split('/');
				 lineOne[x] = splitTxt[0];
				 lineTwo[x] = splitTxt[1];	
				 //CHANGE OUT THE COL ID BELOW FOR THE ONE THAT MATCHES YOUR PARTICULAR MATRIX FIELD ID IN THE DB FOR THE THUMB URL
				 urlThmb[x] = $(this).find('col_id_37').text();
		...
		base.buildNavigation = function() {
			...
			if (base.options.buildNavigation && (base.pages > 1)) {
  			base.$items.each(function(i,el) {
  				var index = i + 1;
  				var $a = $("<a href='#' id='btn" + index + "' onclick='javascript:removeVid();'><img src='" + urlThmb[i] + "' class='thumbImg' border='0'><div class='buttonTxt'>" + lineOne[i] + "<br><b>" + lineTwo[i] +"</b></div></a>");
				//$a.class(btn);
			 //alert(order[i]);
			// alert(lineOne[i]);
			 //alert(urlThmb[i]);
    ...
    // Bind to the forward and back buttons
      ...
      base.$startStop.click(function(e) {
		...
		base.startStop = function(playing) {
			...
			$("#atag3").click(function(){
			base.startStop(false);
									   });
			$("#atag4").click(function(){
			base.startStop(false);
									   });
			$("#atag5").click(function(){
			base.startStop(false);
									   });
			$(".thmb_link").click(function(){
			base.startStop(false);
									   });
