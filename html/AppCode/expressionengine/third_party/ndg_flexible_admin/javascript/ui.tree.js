...
$.widget("ui.tree", {
	...
	expandAll:function(){
		var selftree = this
		node = this.element;
		$('li', node).each(function() { selftree.expand(this); });	
	},
	collapseAll:function(){
		var selftree = this
		node = this.element;
		$('li', node).each(function() { selftree.collapse(this); });	
	},
	...
	_setNodeEvents : function(el) {
		...
		if (this.options.expandOn && this.options.expandOn == this.options.collapseOn) {
			span.bind(this.options.expandOn, function(event) {
				if($(event.target).is("input")){return false }else{
	...
	_getTitle : function(node) {
		...
		var rx=new RegExp("<span[^>]*>(.*?)</span>","ig");
		html=html.replace(rx,"$1");
			
		return $.trim(html.replace(/\n/g, '').replace(/<a[^>]*>/gi, '').replace(/<\/a>/gi, '').replace(/&nbsp;/g,''));
	},
