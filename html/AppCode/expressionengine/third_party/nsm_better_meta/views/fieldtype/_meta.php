...
<div id="nsm_bm_<?= $count; ?>">
	...
	<?= form_hidden("{$input_prefix}[{$count}][default]", $meta["default"]); ?>
	...
	<table>
		<tbody>
			...
			<tr class="<?= ($row_count % 2) ? "odd" : "even"; ?>">
				<td colspan="2">
					<?php
						echo $EE->nsm_better_meta->checkbox(
			...
			<tr class="<?php $row_count++; print($row_count % 2) ? "odd" : "even"; ?>">
				<th scope="row" rowspan="4"><?= lang("geo_metadata") ?></th>
			...
			<tr class="<?php $row_count++; print($row_count % 2) ? "odd" : "even"; ?>">
				...
				<td>
					<?=
						$EE->nsm_better_meta->selectbox(
							$input_prefix."[".$count."][robots_index]",
							array("-- Use channel defaults --" => "", "Yes" => "1", "No" => "0"),
							$meta["robots_index"]
						);
					?>
				</td>
