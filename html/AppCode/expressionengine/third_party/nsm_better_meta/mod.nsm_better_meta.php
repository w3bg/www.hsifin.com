...
class Nsm_better_meta
{
	...
	public function __construct($settings='')
	{
		...
		$NSMBetterMetaObject = new Nsm_better_meta_ext;
		$this->settings = $NSMBetterMetaObject->settings;
	}
	...
	public function template()
	{
		...
		$extension_settings = $this->settings;
		...
		$valid_parameters = array(
			...
			'sitemap_archive', 'sitemap_follow', 'sitemap_include'
		);
		...
		{
			$where = ($entry_id) ? "exp_channel_titles.entry_id = {$entry_id} ": "exp_channel_titles.url_title = '{$url_title}' ";

			if($channel_id)
				$where .= "AND exp_channel_titles.channel_id = {$channel_id} ";

			$where .= "AND exp_channel_titles.site_id = {$site_id} ";
			...
			$sql = "
				SELECT
					...
					exp_channel_titles.dst_enabled					as e_dst_enabled,
				...
				WHERE
					...
					(
						...
						exp_nsm_better_meta.default = 1 OR
			...
			else if (isset($this->EE->config->_global_vars['nsm_lang']))
			{
			...
			if($query_result->num_rows > 0)
			{
				...
				if ($entry_data['meta_keywords_append_default'] && isset($entry_data['keywords']))
				{
					$entry_data['keywords'] .= ", " . $default_metadata['keywords'];
				}
		...
		foreach ($merged_final_metadata as $key => $value)
		{
			if(version_compare(phpversion(), "5.2.3", "<" ))
				$merged_final_metadata[$key] = htmlentities($value, ENT_QUOTES);
			else
		...
		$merged_final_metadata['robots_index']		= ($merged_final_metadata['robots_index'] == TRUE || $merged_final_metadata['robots_index'] == 'index') ? 'index' : 'noindex';
		$merged_final_metadata['robots_follow']		= ($merged_final_metadata['robots_follow'] == TRUE || $merged_final_metadata['robots_follow'] == 'follow') ? 'follow' : 'nofollow';
		$merged_final_metadata['robots_archive']	= ($merged_final_metadata['robots_archive'] == TRUE || $merged_final_metadata['robots_archive'] == 'archive') ? 'archive' : 'noarchive';
		...
		foreach ($merged_final_metadata as $key => $value)
		{
			if(strpos($template, LD.$key.RD) !== FALSE)
			{
	...
	public function entries_xml()
	{
		...
		$this->EE->TMPL->tagparams['disable'] = $this->EE->TMPL->fetch_param('disable') ? $this->EE->TMPL->fetch_param('disable') : "categories|custom_fields|category_fields|member_data|pagination|trackbacks";
		$this->EE->TMPL->tagparams['dynamic'] = $this->EE->TMPL->fetch_param('dynamic') ? $this->EE->TMPL->fetch_param('dynamic') : "off";
		$this->EE->TMPL->tagparams['rdf'] = $this->EE->TMPL->fetch_param('rdf') ? $this->EE->TMPL->fetch_param('rdf') : "off";
		$this->EE->TMPL->tagparams['limit'] = $this->EE->TMPL->fetch_param('limit') ? $this->EE->TMPL->fetch_param('limit') : "500";
		$site_id = isset($tag_params["site_id"]) ? $tag_params["site_id"] : SITE_ID;
	...
	protected function _getEntriesWithMetadata()
	{
		...
		{
			$channelInstance->fetch_custom_weblog_fields();
		}
		...
		$where_sql = "WHERE (nsm_bm.sitemap_include = 1 || nsm_bm.sitemap_include IS NULL) AND";
